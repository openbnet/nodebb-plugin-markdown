...
var Markdown = {
	...
	getLinkTags: function (hookData, callback) {
		...
		// console.log("getLinkTags hookData.links",hookData.links)
	...
	updateParserRules: function (parser) {
		...
		parser.renderer.rules.link_open = function (tokens, idx, options, env, self) {
			...
			if (Markdown.isExternalLink(tokens[idx].attrs[hrefIdx][1])) {
				// console.log("Markdown.isExternalLink",tokens[idx].attrs[hrefIdx][1])
				// hard code to deal with activ forum's [img] link [img] format
				tokens[idx].attrs[hrefIdx][1] = tokens[idx].attrs[hrefIdx][1].replace(/%5B\/img%5D/g,"")
				// console.log("Markdown.isExternalLink after",tokens[idx].attrs[hrefIdx][1])
